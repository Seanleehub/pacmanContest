...
class DefensiveAgent(CaptureAgent):
    ...
    def getDefaultDefendPosition(self, gameState):
        """ Returns the default positon that agent should be defending """
        # Get the default defend position for the first agent
        closestFood = self.getClosestFood(gameState, self.enemyStart, invert=True)
        agentOneDefaultDefence = self.getClosestExit(gameState, closestFood)

        # If the default is on or next to the side of the map, move in a few spaces to cover more boundary
        topCell = (self.half_width - 1, self.height) if self.isRed else (self.half_width, self.height)
        bottomCell = (self.half_width - 1, 0) if self.isRed else (self.half_width, 0)

        if game.manhattanDistance(agentOneDefaultDefence, topCell) <= 3:
            # Move down off border
            for y in range(self.height):
                if self.isRed:
                    if (agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y) in self.friendlyBoundary:
                        if (agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y) not in self.walls:
                            if game.manhattanDistance((agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y), topCell) > 3:
                                agentOneDefaultDefence = (agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y)
                                break
                else:
                    if (agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y) not in self.walls:
                        if (agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y) in self.friendlyBoundary:
                            if game.manhattanDistance((agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y), topCell) > 3:
                                agentOneDefaultDefence = (agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y)
                                break
        if game.manhattanDistance(agentOneDefaultDefence, bottomCell) <= 3:
            # Move up off border
            for y in range(self.height):
                if self.isRed:
                    if (agentOneDefaultDefence[0], agentOneDefaultDefence[1] + y) in self.friendlyBoundary:
                        if (agentOneDefaultDefence[0], agentOneDefaultDefence[1] + y) not in self.walls:
                            if game.manhattanDistance((agentOneDefaultDefence[0], agentOneDefaultDefence[1] + y), bottomCell) > 3:
                                agentOneDefaultDefence = (agentOneDefaultDefence[0], agentOneDefaultDefence[1] + y)
                                break
                else:
                    if (agentOneDefaultDefence[0], agentOneDefaultDefence[1] + y) not in self.walls:
        ...
        # Return the position for first agent
        if self.index == self.teamIndices[0]:
            return agentOneDefaultDefence

        # See if we need to protect a capsule
        agentTwoDefaultDefence = None
        for capsule in self.getCapsulesYouAreDefending(gameState):
            boundary = self.getClosestExit(gameState, capsule)
            delta = self.getMazeDistance(capsule, boundary)
            if delta <= 5:
                agentTwoDefaultDefence = boundary

        # Make sure we aren't defending the same location as our other agent
        if agentTwoDefaultDefence != None:
            closestFood = self.getClosestFood(gameState, self.enemyStart, invert=True)
            boundary = self.getClosestExit(gameState, closestFood)
            if agentTwoDefaultDefence == boundary:
                agentTwoDefaultDefence = self.getClosestExit(gameState, agentTwoDefaultDefence, secondExit=True)
                return agentTwoDefaultDefence

        # Defend the side of the our first agent with the biggest area and sit ~6 spaces away
        for y in range(self.height):
            if self.isRed:
                if game.manhattanDistance((agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y), agentOneDefaultDefence) >= 8:
                    if (agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y) in self.friendlyBoundary:
                        agentTwoDefaultDefence = (agentOneDefaultDefence[0], agentOneDefaultDefence[1] - y)
                else:
                    if (agentOneDefaultDefence[0], agentOneDefaultDefence[1] + y) in self.friendlyBoundary:
                        agentTwoDefaultDefence = (agentOneDefaultDefence[0], agentOneDefaultDefence[1] + y)
            else:
        ...
        # Move off the border to cover more ground
        if game.manhattanDistance(agentTwoDefaultDefence, topCell) <= 3:
            # Move down off border
            for y in range(self.height):
                if self.isRed:
                    if (agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] - y) in self.friendlyBoundary:
                        if (agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] - y) not in self.walls:
                            if game.manhattanDistance((agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] - y), topCell) > 3:
                                agentTwoDefaultDefence = (agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] - y)
                                break
                else:
                    if (agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] - y) in self.friendlyBoundary:
                        if (agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] - y) not in self.walls:
                            if game.manhattanDistance((agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] - y), topCell) > 3:
                                agentTwoDefaultDefence = (agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] - y)
                                break
        if game.manhattanDistance(agentTwoDefaultDefence, bottomCell) <= 3:
            # Move up off border
            for y in range(self.height):
                if self.isRed:
                    if (agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] + y) in self.friendlyBoundary:
                        if (agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] + y) not in self.walls:
                            if game.manhattanDistance((agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] + y), bottomCell) > 3:
                                agentTwoDefaultDefence = (agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] + y)
                                break
                else:
                    if (agentTwoDefaultDefence[0], agentTwoDefaultDefence[1] + y) in self.friendlyBoundary:
